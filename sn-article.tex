...
%the incremental displacement field $\Delta \mathbf{u}$ for the current time step $t_{n+1}$, at the current (outer) iteration, is provided. The values of the following variables are given from the previous time step:
%\begin{equation}
%    \left\{\mathbf{F}_n^p,\ \alpha_n\right\}
%\end{equation}
%The goal is then to have an algorithm to obtain the following variables at time $t_{n+1}$
%\begin{equation}
%    \left\{\mathbf{F}_{n+1}^p,\ \alpha_{n+1},\boldsymbol{\sigma}_{n+1}\right\}
%\end{equation}
...
%\begin{equation}
%    \mathbf{F}_{n+1}^p=exp\left[\frac{3}{2}\Delta\gamma\mathbf{R}_{n+1}^{e^T}\frac{{\bb{\tau}}_{d_{n+1}}}{\tau^{eq}_{n+1}}
%    \mathbf{R}_{n+1}^e\right]\mathbf{F}_n^p
%\end{equation}
%Due to plastic isotropy, this equation can be written as:
%\begin{equation}
%    \mathbf{F}_{n+1}^p=\mathbf{R}_{n+1}^{e^T}exp\left[\frac{3}{2}\Delta\gamma\frac{{\boldsymbol{\tau}_d}_{n+1}}{\tau^{eq}_{n+1}}\right]\mathbf{R}_{n+1}^e\mathbf{F}_n^p
%\end{equation}
...
%\begin{equation}
%    \mathbf{F}_{n+1}^e={\mathbf{F}_{n+1}^{e\ trial}\mathbf{R}}_{n+1}^{e^T}exp\left[-\frac{3}{2}\Delta\gamma\frac{{\boldsymbol{\tau}_d}_{n+1}}{\tau^{eq}_{n+1}}\right]\mathbf{R}_{n+1}^e
%\end{equation}
%where $\mathbf{F}_{n+1}^{e\ trial}$ is given by:
%\begin{equation}
%\mathbf{F}_{n+1}^{e\ trial}=\mathbf{f}_{n+1}\mathbf{F}_{n+1}^{e}
%\end{equation}
%$\mathbf{f}_{n+1}$ is the relative deformation gradient (equation 2.45). 
%Using the fact that $\mathbf{F}^e=\mathbf{V}^{e}\mathbf{R}^{e}$
%\begin{equation}
%    \mathbf{V}_{n+1}^e\mathbf{R}_{n+1}^e={\mathbf{F}_{n+1}^{e\ trial}\mathbf{R}}_{n+1}^{e^T}exp\left[-\frac{3}{2}\Delta\gamma\frac{{\boldsymbol{\tau}_d}_{n+1}}{\tau^{eq}_{n+1}}\right]\mathbf{R}_{n+1}^e
%\end{equation}
%Multiplying both sides by $\mathbf{R}^{e^T}_{n+1}$
%\begin{equation}
%    \mathbf{V}_{n+1}^e={\mathbf{F}_{n+1}^{e\ trial}\mathbf{R}}_{n+1}^{e^T}exp\left[-\frac{3}{2}\Delta\gamma\frac{{\boldsymbol{\tau}_d}_{n+1}}{\tau^{eq}_{n+1}}\right]
%\end{equation}
%Multiplying by the inversion of the exponential term:
%\begin{equation}
%    \mathbf{V}_{n+1}^{e}exp\left[\frac{3}{2}\Delta\gamma\frac{{\boldsymbol{\tau}_d}_{n+1}}{\tau^{eq}_{n+1}}\right]={\mathbf{F}_{n+1}^{e\ trial}\mathbf{R}}_{n+1}^{e^T}
%\end{equation}
%Taking the square of both sides:
%\begin{equation}
%    \mathbf{V}_{n+1}^{e}\ exp\left[3\Delta\gamma\frac{{\boldsymbol{\tau}_d}_{n+1}}{\tau^{eq}_{n+1}}\right]\mathbf{V}_{n+1}^{e}={\mathbf{F}_{n+1}^{e\ trial}\mathbf{R}}_{n+1}^{e^T}{\mathbf{F}_{n+1}^{{e\ trial}^T}\mathbf{R}}_{n+1}^{e}
%\end{equation}
%
%Isolating the RHS of this equation and using the fact that $\mathbf{R}^{e^T}_{n+1}\mathbf{R}^{e}_{n+1}=\textbf{I}$:
%\begin{equation}
%    {\mathbf{F}_{n+1}^{e\ trial}\mathbf{R}}_{n+1}^{e^T}{\mathbf{F}_{\mathbf{n}+1}^{{e\ trial}^T}\mathbf{R}}_{n+1}^e=\mathbf{F}_{n+1}^{e\ trial}\mathbf{F}_{n+1}^{{e\ trial}^T}=\left(\mathbf{V}_{n+1}^{e\ trial}\right)^\mathbf{2}
%\end{equation}
%
%Subbing this into equation 3.47 we get:
%\begin{equation}
%    \mathbf{V}_{n+1}^e exp\left[3\Delta\gamma\frac{{\boldsymbol{\tau}_d}_{n+1}}{\tau^{eq}_{n+1}}\right]\mathbf{V}_{n+1}^e=\left(\mathbf{V}_{n+1}^{e\ trial}\right)^\mathbf{2}
%\end{equation}
%
%By rearranging and taking the square root of both sides, the following can be obtained:
%\begin{equation}
%    \mathbf{V}_{n+1}^e=\mathbf{V}_{n+1}^{e\ trial}exp\left[-\frac{3}{2}\Delta\gamma\frac{{\boldsymbol{\tau}_d}_{n+1}}{\tau^{eq}_{n+1}}\right]
%\end{equation}
%
%Taking the natural log of both sides:
%\begin{equation}
%    ln\left(\mathbf{V}_{n+1}^{e}\right)=ln\left(\mathbf{V}_{n+1}^{e\ trial}exp\left[-\frac{3}{2}\Delta\gamma\frac{{\boldsymbol{\tau}_d}_{n+1}}{\tau^{eq}_{n+1}}\right]\right)
%\end{equation}
%
...
%\begin{equation}
%    \boldsymbol{\varepsilon}_{n+1}^{e}=\boldsymbol{\varepsilon}_{n+1}^{e\ trial}-\frac{3}{2}\Delta\gamma\frac{{\boldsymbol{\tau}_d}_{n+1}}{\tau^{eq}_{n+1}}
%\end{equation}
...
%	F_p = \tau^{eq}_{n+1} - \sigma_{y} \left(\alpha_{n+1}\right)=0 \\
%	\boldsymbol{\varepsilon}_{n+1}^e = \boldsymbol{\varepsilon}_{n+1}^{e\ trial}
%		- \frac{3}{2} \Delta \gamma \frac{{\boldsymbol{\tau}_d}_{n+1}}{\tau^{eq}_{n+1}} \\
%		\alpha_{n+1}=\alpha_n+\Delta \gamma
%	\end{array}\right.
%\end{equation}
...
%\begin{equation}
%    \tau^{eq}_{n+1}-\sigma_y\left(\alpha_{n+1}\right)=0
%\end{equation}
%
...
%\begin{eqnarray}
%\tau^{eq}_{n+1}&=&\sqrt{\frac{3}{2}}\left\|2 \mu \operatorname{dev}\left(\boldsymbol{\varepsilon}_{n+1}^e\right)\right\| \\ \notag
%&=&
%\sqrt{\frac{3}{2}}\left\|2 \mu \operatorname{dev}\left(\boldsymbol{\varepsilon}_{n+1}^{e \text { trial }}-\frac{3}{2}\Delta \gamma\frac{{\boldsymbol{\tau}_d}_{n+1}}{\tau^{eq}_{n+1}}\right)\right\|
%\end{eqnarray}
%
%Given that:
%\begin{equation}
%\left\|2 \mu \operatorname{dev}\left(\frac{3}{2} \Delta \gamma \frac{{\boldsymbol{\tau}_d}_{n+1}}{\tau^{eq}_{n+1}}\right)\right\|=3 \mu \Delta \gamma
%\end{equation}
%The von Mises equivalent stress can be expressed as:
%\begin{equation}
%\tau^{eq}_{n+1}=\sqrt{\frac{3}{2}}\left\|2 \mu \operatorname{dev}\left(\boldsymbol{\varepsilon}_{n+1}^{e \text { trial }}\right)\right\|-3 \mu \Delta \gamma
%\end{equation}
%The following relation for the equivalent stress can therefore be obtained:
%\begin{equation}
%	\tau^{eq}_{n+1}=\tau^{eq}_{{n+1}^{\text{trial}}} - 3 \mu \Delta \gamma
%\end{equation}
...
\begin{equation}
%	\boldsymbol{\sigma}_{n+1} = \frac{1}{J} \mathbf{D}^e:\boldsymbol{\varepsilon}^{e} \nonumber
	\boldsymbol{s}^{[m+1]}
	=
	(\sfrac{1}{J}) 2\mu \;\text{dev}\left(\boldsymbol{\varepsilon}_{e}^{[m+1]}\right) \nonumber
\end{equation}

(v) Solve the pressure Poisson's equation (Equation \ref{eq:pressureEqn}) to calculate the hydrostatic pressure $p^{[m+1]}$, where $J^{[m+1]} = -\kappa \; \text{tr}\left(\boldsymbol{\varepsilon}_{e}^{[m+1]}\right)$.
...
The constitutive equations for the Lemaitre model can then be given in terms of the spatially rotated plastic stretch $\bar{\mathbf{d}}_p$ (or plastic deformation gradient rate $\dot{\mathbf{F}_p}$), the hardening variable rate $\dot{\alpha}$, and damage variable rate $\dot{D}$:
 \begin{equation} \label{eq:lemaitre_dp}
%	\bar{\mathbf{d}}_p = 
%	\dot{\gamma}\frac{\partial F}{\partial\boldsymbol{\tau}}=\frac{3}{2}\frac{\dot{\gamma}}{\left(1-D\right)}\frac{\text{dev}		
\bar{\mathbf{d}}_p = 
	\dot{\mathbf{F}_p} \mathbf{F}_p^{-1} =
	\frac{3}{2} \frac{\dot{\gamma}}{1 - D} \mathbf{R}_e^{T} \frac{\text{dev}(\boldsymbol{\tau})}{\sigma_{v}} \mathbf{R}_e
%	\frac{3}{2} \frac{\dot{\gamma}}{1 - D} \mathbf{R}_e^{T} \frac{\text{dev}(\boldsymbol{\tau})}{\sigma_{v}} \mathbf{R}_e
%(\boldsymbol{\tau})}{\sigma_{v}}	
\end{equation}
 \begin{equation}
...
\begin{equation}
\begin{aligned}
%\frac{\tau^{eq}_{n+1}}{1-D^{i}_{n+1}}=\bar{\tau}_{{eq}_{n+1}}&=\sqrt{\frac{3}{2}}\left\|2 \mu \operatorname{dev}\left(\boldsymbol{\varepsilon}_{n+1}^e\right)\right\|\\
%&=\sqrt{\frac{3}{2}}\left\|2 \mu \operatorname{dev}\left(\boldsymbol{\varepsilon}_{n+1}^{e \text { trial }}-\frac{3}{2} \frac{\Delta \gamma}{\left(1+D^{i}_{n+1}\right)} \frac{\boldsymbol{\tau}_{d_{n+1}}}{\tau_{eq\ n+1}}\right)\right\|
fix
\end{aligned}
\end{equation}
...
\subsubsection{Non-local damage}
...
\subsubsection{Crack-closure effects}
...
\end{equation}
  \eIf{$f_{trial}>0$}{
   ...
   set $\Delta {\varepsilon}_{n+1}^h$, $\Delta {\varepsilon}_{n+1}^q$ and $\Delta \overline{{\varepsilon}}^p_{n+1}$ to 0. Go to step iv 
  }
...
\end{equation}
%\begin{equation}
%\boldsybol{\tau}_{n+1}=\left(q_{trial}-3\mu\Delta \boldsymbol{\varepsilon}_{n+1}^q\left(\frac{3\mathbf{s}_{trial}}{2q_{trial}}\right)\right)+\left(p_{trial}-K\Delta\varepsilon^{h}_{n+1}\right)\nonumber
%\end{equation}
...
%\begin{equation}
%\boldsymbol{\varepsilon}^{e}_{n+1}=\boldsymbol{\varepsilon}^{e}_{trial}-\sqrt{\frac{3}{2}}\Delta\gamma{\frac{dev(\boldsymbol{\varepsilon}^{e}_{trial})}{\lvert\lvert\boldsymbol{\varepsilon}^{e}_{trial}\rvert\rvert}}\nonumber
%\end{equation}
%\begin{equation}
%\boldsymbol{\tau}_{n+1}=g_e(d)\mathbf{D}^e:\boldsymbol{\varepsilon}^{e}_{n+1}\ 
