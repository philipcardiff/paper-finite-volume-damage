...
The developed methods are validated against \emph{flat notched bar} and \emph{notched round bar} experimental cases, and subsequently applied to the analysis of axisymmetric wire drawing.
...
Nonetheless, for continuous processes such as wire drawing, Lagrangian approaches require the user to select a a sufficiently long initial domain (e.g. initial wire length) such that steady state phenomena of interest can be examined.
Eulerian approaches do not have such a limitation as the workpiece \emph{inlet} and \emph{outlet} (e.g. wire upstream and downstream boundaries) as they are static in space.
In a bid to combine the advantages of both, the current work proposes a Lagrangian approach with Eulerian-style boundary conditions, which allow the wire upstream and downstream boundaries to stay fixed in space.
...
All implementations are in the solids4foam toolbox \cite{Cardiff2018, Tukovic2018} for the open-source C++ softeware OpenFOAM, building on previous work related to metal forming \cite{cardiff_lagrangian_2017, clancy_improving_2019}.
...
Although the total Lagrangian approach is a viable option for wire drawing, the current work adopts the updated Lagrangian approach as developing Eulerian-type upstream and downstream conditions is conceptually easier in an updated Lagrangian formulation, as described in Section \ref{sec:euler_BCs}.
...
This Laplacian term can be considered as an approximate compact-stencil linearisation of the surface stress term.
This allows the coupled vector system to be temporarily decoupled in three scalar component equations.
This three scalar component equations can then be solved independently, where outer Picard iterations provide the inter-component coupling.
...
Nonetheless, as noted previously \citep{cardiff_block-coupled_2016}, segregated solution procedures can require significantly more outer iterations and slow convergence relative to coupled approaches, particularly in high aspect ratio linear cases.
In the current work, 
...
%	&=& \sum_{F}\int_{\mathrm{\Gamma_{uf}}}(j\bb{f}^{-T}\cdot{\bb{n}_u)\cdot\boldsymbol{\sigma}}d\Gamma_{uf}
	...
	\left(j\boldsymbol{\sigma}\cdot\bb{f}^{-T}\right)_f
\end{equation}
...
As is well-known, displacement-only formulations are susceptible to numerical hydrostatic pressure oscillations in regions of large isochoric plastic strains.
...
\end{equation}
where $p = - \text{tr}(\bb{\sigma})/3$ is the hydrostatic pressure and the trace operator is indicated by $\text{tr}(\bullet)$.
...
\begin{equation}
a_p \Delta \bb{u}_p+\sum_F a_n \Delta \bb{u}_n=\bb{b}_p
\end{equation}

Where $a_{p}$ is the central coefficient, $a_{n}$ are the coefficients associated with the centre of neighbouring cells, $F$ is the number of internal faces of the control volume,  $\bb{b}_p$ is the source vector contribution.
...
Where $\bb{K}$ is an $M \times M$ coefficient matrix containing the implicit operators where $M$ is the total number of control volumes. The solution vector $\bb{u}$ contains the unknown cell-centre displacement increments $\Delta\bb{ u}$. $\bb{f}$ is the source term containing the explicit operators.
...
\end{equation}
\begin{equation}
\bb{B}_{e}^{\text{trial}} = \bb{f}^{[m+1]} \cdot \bb{B}_{e}^{[m]} \cdot (\bb{f}^{[m+1]})^{T}\nonumber
\end{equation}
\begin{equation}
...
% can then be given in terms of the spatially rotated plastic stretch $\bar{\bb{d}}_p$ (or plastic deformation gradient rate $\dot{\bb{F}_p}$), the hardening variable rate $\dot{\alpha}$, and damage variable rate $\dot{D}$:
 \begin{eqnarray} \label{eq:lemaitre_dp}
    ...
    \dot{\bb{F}}_p \cdot \bb{F}_p^{-1}
    &=&
    \frac{\dot{\bar{\varepsilon}}_p}{1 - D} \; \bb{R}_e^T \cdot 
    \left[
    \frac{3}{2} \frac{\text{dev}\left(\boldsymbol{\sigma} \right)} {\sigma_{v}}
    \right]
    \cdot  \bb{R}_e \\
...
\citet{gurson_continuum_1977} proposed the canonical micro-mechanical framework for ductile damage prediction and provides the basis for various derived models \citep{besson_continuum_2010, bettaieb_numerical_2011, achouri_numerical_2013, cao_models_2017, tekkaya_damage_2020}.
...
\begin{algorithm}[htbp] \label{alg:GTN} \small
\SetAlgoLined
(i) Update deformation gradients for a given incremental displacement
...
\end{equation}
\begin{equation}
\bb{B}^{\text{trial}}_e =\bb{f}^{[m+1]}\bb{B}_{e}^{[m]}(\bb{f}^{[m+1]})^{T}\nonumber
\end{equation}
\begin{equation}
...
\end{equation}
where $\psi_e \left(\bb{\varepsilon}^e\right)$ is the current elastic energy contribution, $\bar{\psi}_e \left(\bb{\varepsilon}^e\right)$ is a history variable which gives the largest value reached by the elastic energy contribution in time, 
...
This section assesses the performance of the developed finite volume procedures on two benchmark test cases: (i) 2-D axisymmetric notched round bar and (ii) 3-D flat notched bar. %and (iii) 2-D axisymmetric wire drawing.
...
Unless stated otherwise, a global momentum Rhie-Chow scale factor $\mathcal{R} = 0.01$ is used in all of the following finite volume simulations, and the pressure Rhie-Chow scale factor was $\mathcal{R}_p = 10^3$.
...
The 3-D geometry consists of $152.4 \times 25.4 \times 2.37$ mm plate with 4.06 mm diameter side notches.
...
An adaptive time-stepping procedure was employed for the Abaqus finite element cases, with a maximum time increment of $1$ ms and a minimum of \hl{XXX} ms.
...
It is worth noting that the reaction forces for both the \citet{clancy_improving_2019} implementation and the implementation in this work (which both use the logarithmic strain) are slightly less than the \citet{cardiff_lagrangian_2017}  and the Abaqus predictions in the latter stages of the deformation.
...
\begin{figure}[htbp]
	\centering
		...
		{
			\label{airbus}\includegraphics[height=0.45\textwidth]{./Figures/finiteVolumeImplementation/inhomogenousDeformation/forceDispAxi}
		}
		\subfigure[Flat notched bar]
		{
			\label{airbus}\includegraphics[height=0.45\textwidth]{./Figures/finiteVolumeImplementation/inhomogenousDeformation/forceDispFlat}
		}
...
The value is seen to increase smoothly In the notched round bar case (Figure \ref{fig:eps_round_bar}), whereas there is an initial elastic region followed by a stepper slope in the flat notched bar case (Figure \ref{fig:eps_flat_bar}).
...
%\end{center}
	\centering
		\subfigure[Notched round bar]
		{
			\includegraphics[width=0.45\textwidth]{./Figures/plasticityCompare/axiCompare/disp_EpsPEq.png}
			\label{fig:eps_round_bar}
		}
		\subfigure[Flat notched bar] 
		{
			\includegraphics[width=0.45\textwidth]{./Figures/plasticityCompare/bordenCompare/disp_EpsPEq.png}
...
After verifying the elastoplasticity procedure, the notched round and flat bar test cases are examined using the proposed damage models.
Comparisons are given against results available in the literature and those generated using Abaqus.
...
Comparing the OpenFOAM and Abaqus reaction force predictions (Figure \ref{fig:lemaitre_forces}), the OpenFOAM and Abaqus predictions can be seen to agree closely. 
...
The Abaqus simulations struggled to converge in the rapid crack propagation stage of the flat notched bar case, requiring very small time increments until it eventually crashes before reaching the end time.
%of $~0.463$ mm.
In contrast, no such issues were encountered with the OpenFOAM simulation.
...
\begin{figure}[htbp]
	\centering
		\subfigure[Notched round bar]
		{
			\includegraphics[height=0.3\textwidth]{./Figures/LemaitreCompare/axiCompare/1.1LemaitreCompare.png}
		}
		\qquad
		\subfigure[Flat notched bar]
		{
			\includegraphics[height=0.3\textwidth]{./Figures/LemaitreCompare/borden/bordenDispForce.png}
		}
...
Similiar to the forces, the local values for damage and equivalent plastic strain can be seen to closely agree between OpenFOAM and Abaqus (Figure \ref{fig:lemaitre_point_values}).
Nonetheless, upon examining the figures closely, the OpenFOAM damage and strain are observed to increase slightly quicker than those from Abaqus.
...
\begin{figure}[htbp]
	\centering
		\subfigure[Non-local damage vs.displacement (mm)]
		{
			\includegraphics[width=0.3\textwidth]{./Figures/LemaitreCompare/axiCompare/disp_damageNonLocal.png}
			}
		\subfigure[Equivalent plastic strain vs. displacement]
		{
			\includegraphics[width=0.3\textwidth]{./Figures/LemaitreCompare/axiCompare/disp_EpsPEq.png}
		}
		\caption{Abaqus and OpenFOAM implementations comparison}
	\label{fig:lemaitre_point_values}
\end{figure}

Figure \ref{fig:lemaitre_damage_field} compares the predicted non-local damage distribution at a loading displacement of $1.2$ mm with the predictions provided in \citet{cesar_de_sa_damage_2006}.
Close agreement can be observed between the distributions and values can be observed.
...
\begin{figure}[htbp]
	\centering
		...
		\qquad
		\subfigure[Results from \cite{cesar_de_sa_damage_2006}]
		{
			\includegraphics[height=0.4\textwidth]{./Figures/LemaitreCompare/updated_deSaNonLocal2}
		}
		\caption{Comparison of non-local damage distribution \hl{mirror foam results; use same scale}}
...
Here, the simulations are repeated for different values of the material damage parameter $S_0$ to demonstrate its effect.
...
\begin{figure}[htb]
	\centering
		\subfigure[Displacement vs. non-local Damage] {\label{airbus}\includegraphics[width=0.4\textwidth]{./Figures/LemaitreCompare/borden/disp_damageNonLocal.png}}
		\subfigure[Displacement vs. equivalent plastic strain] {\label{boeing} \includegraphics[width=0.4\textwidth]{./Figures/LemaitreCompare/borden/disp_EpsPEq.png}}
...
%%%%
\subsubsection{Effect of the Effective Non-Local Damage Parameter}
%%%%
...
As with the simulations conducted with the Lemaitre model, quicker localisation can be observed in the OpenFOAM implementation.
A feature of this test case is the sharp crack propagation (Figure 5.23).
The Abaqus simulation has convergence issues and eventually crashes at a displacement of approximately $0.371$ mm.
Once again, no convergence issues were encountered in the OpenFOAM simulations.
As noted above, the superior convergence abilities of the OpenFOAM simulations are likely because of the segregated solution procedure.
...
\begin{figure}[t!]
	\centering
		\subfigure[Porosity vs. displacement] {\label{airbus}\includegraphics[width=0.4\textwidth]{./Figures/GTNCompare/epsilonPEq_GTN.jpg}}
		\subfigure[Equivalent plastic strain vs. displacement] {\label{boeing} \includegraphics[width=0.4\textwidth]{./Figures/GTNCompare/displacement_porosity.jpg}}
...
This section applies the developed damage procedures to the wire drawing metal forming process (Figure \ref{fig:wireDrawing}).
The key parameters in the wire drawing process are the die semi-angle $\alpha$ (in $^{\circ}$), which is the angle made by the die with the wire axis, and the reduction ratio $r$ ($\%$), given by
\begin{equation}
...
This case is not expected to displacement catastrophic fracture; however, damage accumulation is expected.
...
Table \ref{tab:wire_Draw_geom} summarises the geomety and mesh parameters.
...
A modified Voce Law \cite{cao_modelling_2014} is chosen to describe the plastic hardening law, as Cao \cite{cao_modelling_2014} found that this law could well describe the hardening behaviour of high carbon steel in tensile, torsion and compression tests:
...
\end{equation}
The die elastic parameters (Table \ref{tab:die_draw_material_properties}) correspond to tungsten carbide, as is typically used for die-drawing nibs.
	...
	\centering
		\begin{tabular}{lllll} \hline
   		...
   		Hardening law & $689+(1340-689+250\,\bar{\varepsilon}_p)(1-e^{-32.82\,\bar{\varepsilon}_p})$ & MPa  \\
			\hline
		\end{tabular}
	\caption{Wire elastoplastic material parameters}
...
A back tension (\hl{how much?}) is applied to the wire upstream boundary, while a total drawing displacement of 30 mm is applied to the downstream wire boundary over 150 ms.
...
For the Lemaitre model with crack-closure effects, the crack-closure parameter is taken to be 0.2 \cite{desmorat_modeling_2008, lemaitre_course_1996, bouchard_enhanced_2011}.
\begin{table}[htb]
	\centering
		\begin{tabular}{llll} \hline
			Property & Symbol & Value & Units \\ \hline 
...
As a consequence of the triaxiality cut-off $\left(-\sfrac{1}{3}\right)$ for damage evolution, there is limited damage evolution towards the upper area of the wire.
In contrast, the damage is correctly predicted at a maximum in the centre of the wire for the Lemaitre model with crack-closure effects (lower image in Figure \ref{fig:lemaitre_wire_draw_damage}).
This is because this is where the triaxiality and, relatedly, the positive stress tensor are at a maximum.
\begin{figure}[htbp]
	\centering
...
%%%%
The GTN model wire parameters are given in Table \ref{tab:GTN_wire_draw_mat}.
\begin{table}[htb]
	\centering
		\begin{tabular}{llll} \hline
		Property & Symbol & Value & Units  \\ \hline 
...
The GTN model accurately predicts the porosity to be at maximum at the centre of the wire (Figure \ref{fig:GTN_wire_draw_poro}).
This is because porosity evolution due to nucleation is only set to occur when the hydrostatic pressure is negative (equation \ref{eqn:nucleatonA}) \hl{huh... check this}.
...
It can be observed that the Gaussian assumption leads to the porosity saturating at a certain value, after which it does not evolve. This is unlikely to reflect the true material behaviour \cite{cao_modelling_2014}. 
...
%%%%
The phase field model wire parameters are given in Table \ref{tab:PF_wire_draw_mat}.
\begin{table}[htb]
	\centering
		\begin{tabular}{llll} \hline
			Property & Symbol & Value & Units  \\ \hline 
...
The phase field fracture model predicts material degradation to be at its greatest towards the upper part of the wire (Figure \ref{fig:PF_wire_draw}).
...
The phase field fracture model does not distinguish between tensile and compressive stress states for the plastic contribution towards crack growth, leading to this unrealistic behaviour.
\begin{figure}[htbp]
	\centering
		\subfigure[Distribution for $d$]
		{
		%\label{airbus}
		\includegraphics[width=0.8\textwidth]{./Figures/SimulationAndAnalysis/modelCompare/phaseFieldExample.png}
		}
	  \qquad
		\subfigure[Equivalent plastic strain distribution]
		{
		%\label{boeing}
		\includegraphics[width=0.8\textwidth]{./Figures/SimulationAndAnalysis/modelCompare/phaseFieldEpsPEq.png}
		}
...
%%%%
\subsubsection{Comparison of Damage Models Approaches}
%%%%
...
This is possible (for example, using X-ray tomography measurements such as in \citet{thuillier_ductile_2012} and \citet{fansi_numerical_2013}), but it is not trivial.
...
To test the Lemaitre model further, its ability to predict \emph{chevron cracks} is examined in this section.
Chevron cracks, also known as central burst defects, are common defects that occur at the centre of the wire in drawing (Figure \ref{fig:chevronCracks}).
...
\hl{compare geom to previous; is it the same?}
\begin{table}[htb]
	\centering
		\begin{tabular}{llll} \hline
		    %Loading Conditions \\ \hline 
...
Using the developed effective non-local damage scheme, the accurate modelling of chevron cracks is evident in Figure \ref{simulatedChevronCrack}.
In contrast, in Figure \ref{simulatedDamageNotLimited}, the results can be seen if the growth of the local damage field $D$ is not restricted when $\bar{D}_{eff}>D_c$ (step (i) in Algorithm \ref{effectiveNonLocalDamage}). The unrestricted growth of the local damage field $D$ leads to the rapid growth of the $\bar{D}$ field, and therefore a large number of cells have a value for $\bar{D}$ which exceeds $D_c$.
This results in rapid crack propagation and an inability to predict the formation of chevron cracks (Figure \ref{simulatedDamageNotLimited} b). 
\begin{figure}[htbp]
	\centering
...
In this section, the experimental high-carbon steel wire drawing series from \citet{roh_process_2021} is used to investigate the ability of the Lemaitre-based damage models to predict fracture in wire drawing accurately.
...
Table \ref{tab:roh_draw_series} summarises the experimental drawing test data available, where tests which displayed catastrophic wire fracture are indicated with the symbol $X$.
\begin{table}[htb]
	\centering
		\begin{tabular}{llll} \hline
...
A $30$ mm long wire was used for drawing simulations, aside from the cases where $r=36\%$ and $\alpha=4^{\circ}$ and $r=20\%$ and $\alpha=2^{\circ}$.
In these cases, a longer 50 mm wire was used to ensure minimum upstream and downstream boundary effects.
The die and wire geometry is summarised in Table \ref{tab:drawing_series_die_geom}.
...
A total drawing displacement of 30 mm is applied to the wire downstream boundary in increments of 0.1 mm over a time step of 0.5 ms (the total simulation time was 150 ms).
A back tension of \hl{XXXX} was applied to the wire upstream boundary.
%All other die surfaces are assumed to have a zero displacement condition \hl{check}.
As before, to avoid localisation in the initial time steps, the damage is set to begin evolving after a drawing displacement of $2$ mm.
%\begin{table}[htb]
%	\centering
%		\begin{tabular}{llll} \hline
%		    %Loading Conditions \\ \hline 
...
In the models presented here, the non-local gradient equation is not incorporated as localisation behaviour is limited due to the low calibrated value of $D_c$.
...
The calibration procedure follows that of \citet{masse_study_2010}, where the plastic hardening law is first calibrated against the experimental force-displacement data up until the point where necking can is observed ($\approx6.5\%$ engineering strain).
Subsequently, the damage parameters are calibrated while the elastoplastic parameters are fixed.
By separating out the calibration of the plastic hardening law and the damage parameters, the calibration procedure is simplified, and the risk of over-fitting is reduced.
...
A global-local approach is taken to calibrate the elastoplastic and damage parameters.
To calibrate the elastoplastic parameters, 50 sets of values for $\sigma_{\infty}$, $k$ and $\beta$ (Equation \ref{eqn:modifiedVoce}) are taken from the range of values given in Table \ref{tab:material_model_calibration} using Latin hypercube sampling \cite{loh_latin_1996}.
...
%	\label{tab:material_properties}
%\end{table}
A tensile test is simulated using each of these sets of values. The force-displacement curves that result from each of these simulations are then compared to the experimental data for up to 6.5\% engineering strain.
The results with the minimum difference from this \emph{global} sampling are then further optimised using the \emph{local} Nelder-Mead method \cite{luersen_globalized_2004} to refine the material parameters.
...
As before, the results with the minimum difference from this \emph{global} sampling are then further optimised using the \emph{local} Nelder-Mead method \cite{luersen_globalized_2004} to refine the material parameters (Table \ref{tab:material_model_calibration_lemaitre}).
...
\begin{table}[htb]
	\centering
		\begin{tabular}{llllll} \hline
			...
			\hline
		\end{tabular}
	\caption{Lemaitre damage parameter search ranges and optimal calibrated values}
...
\begin{center}
	...
	\caption{Comparison of the experimental tensile test curve and the calibrated elastoplastic Lematire material laws}
...
The evolution of the equivalent plastic strain, triaxiality and damage for a given cell near the wire axis is provided in Figure \ref{fig:wire_draw_other_results}.
...
It can be observed that the damage evolution primarily occurs with the increase in triaxiality towards the latter stage of its plastic straining.
\begin{figure}[htb]
\begin{center}
...
\begin{figure}[htbp]
	...
	{
%		\label{airbus}
		\includegraphics[width=0.3\textwidth]{./Figures/SimulationAndAnalysis/compare36_12/36_6tauPositive.png}
	}
	\qquad
  	\subfigure[Triaxiality $\eta$]
	{
%		\label{boeing}
		\includegraphics[width=0.3\textwidth]{./Figures/SimulationAndAnalysis/compare36_12/36_6Triaxiality.png}
	}
        \qquad
  	\subfigure[Axial (xx) component of the plastic strain increment tensor]
	{
%		\label{boeing}
		\includegraphics[width=0.3\textwidth]{./Figures/SimulationAndAnalysis/compare36_12/36_6Dpxx.png}
	}	
...
In Figure \ref{fig:final_damage}, the damage accumulated by the cell that is at the wire centre and has exited the process zone (the region that is undergoing plastic straining) at the final time step is compared with the critical damage $D_c$ and plotted for each case.
\hl{Values over 1 mean fracture is predicted}.
It can be observed that both the classic and the proposed formulations of the Lemaitre model are reasonably consistent with the experimental results.
...
\begin{figure}[htbp]
	\centering
		...
		{
%			\label{airbus}
			\includegraphics[width=0.49\textwidth]{./Figures/SimulationAndAnalysis/compareExperimentalSimulation/20Reduction.png}
		}
	  \qquad
  		\subfigure[$36\%$ Reduction]
		{
%			\label{boeing}
			\includegraphics[width=0.49\textwidth]{./Figures/SimulationAndAnalysis/compareExperimentalSimulation/36Reduction.png}
		}
...
Limitations:
- sensitivity with respect to $D_c$;
- friction law and coefficient;
- positive stress tensor dependency.
%There are various factors that could explain the differences between simulated and experimental results. One of these is experimental error, it can be observed in Figure 7.14 that there would be a sharp difference in the obtained $D_c$ if there was only a small difference in the engineering strain to fracture. It's worth noting that in \citet{roh_process_2021}, the critical fracture value was selected at $10\%$ engineering strain for the Ko fracture criterion. In this thesis, the critical damage value was selected at $10.7\%$ engineering strain in order to be consistent with the experimental engineering stress-strain curve provided in \citet{roh_process_2021}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\hl{To be updated with MaREI and other}
This work has emanated from research conducted with the financial support of/supported in part by a grant from Science Foundation Ireland (SFI) under Grant number RC2302\_2 and SFI NexSys 21/SPP/3756. Financial support is gratefully acknowledged from the Irish Research Council through the Laureate program, grant number IRCLA/2017/45. Vikram Pakrashi would also like to acknowledge FlowDyn RDD/966 and SiSDATA EAPA\_0040/2022. Additionally, the authors want to acknowledge project affiliates, Bekaert, through the Bekaert University Technology Centre (UTC) at University College Dublin (\url{www.ucd.ie/bekaert}), and I-Form, funded by Science Foundation Ireland (SFI) Grant Number 16/RC/3872, co-funded under European Regional Development Fund and by I-Form industry partners.
