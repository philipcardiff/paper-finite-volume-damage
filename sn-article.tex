...
\begin{equation}
\begin{aligned}
%\frac{\tau_{eq}_{\ n+1}}{1-D^{i}_{n+1}}=\bar{\tau}_{{eq}_{n+1}}&=\sqrt{\frac{3}{2}}\left\|2 \mu \operatorname{dev}\left(\boldsymbol{\varepsilon}_{n+1}^e\right)\right\|\\
...
\textbf{\subsubsection{Elastic model}}
...
\textbf{\subsubsection{Elasto-plastic model}}
...
\ \ \text{       \textbf{while} convergence criteria not met \textbf{do}} 
\ \text{       - Assemble system of linear equations (equation 2.48), discretising each term}
\ \text{         using equations 2.50 to 2.54 and and solve for $\Delta\mathbf{u}$}
\  \text{      - Update kinematics and stress using given constitutive law e.g. Algorithm 2 (Chapter 3)}
\ \ \text{       \textbf{end while}}

\ \ \text{      - Interpolate cell-centre displacements to the vertices}

\ \ \text{      - Move mesh to the deformed configuration using the vertex displacements}
